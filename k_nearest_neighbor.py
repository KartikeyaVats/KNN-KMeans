        ...
        for feature in features:
            for train_feature in self.train_features:
                if self.distance_measure == "euclidean":
                    dist_or_sim = euclidean(feature, train_feature)
                else:
                    dist_or_sim = cosim(feature, train_feature)
                distances.append(dist_or_sim)
            neighbors = sorted(range(len(distances)), key=distances.__getitem__)[:self.n_neighbors]
            labels_relevant = [self.train_targets[neighs][0] for neighs in neighbors]
